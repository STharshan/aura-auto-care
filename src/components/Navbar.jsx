    ...
    <>
      ...
      <header
        className={`fixed top-0 left-0 w-full z-9999 transition-all duration-500 ${
          scrolled || isOpen ? "bg-black shadow-md" : "bg-transparent"
        }`}
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-2 flex justify-between items-center text-white">
          {/* Logo */}
          ...
          <div className="hidden xl:flex items-center">
            <HashLink to="/#contact">
              <button className="px-7 py-2.5 bg-linear-to-br from-[#e80202] to-[#a80000] cursor-pointer hover:opacity-90 text-white font-medium text-base rounded-lg border-2 border-white/15 shadow-[0_6px_25px_rgba(255,0,0,0.4)] transition-all duration-300">
                Get In Touch
              </button>
            </HashLink>
          </div>
          ...
          <div className="xl:hidden">
            <button
              ...
              className="focus:outline-none text-white p-1"
      ...
      {/* ── MOBILE MENU — full-screen overlay, separate from header ── */}
      {isOpen && (
        <div
          className="xl:hidden fixed inset-0 z-9998 bg-black text-white flex flex-col"
          style={{ paddingTop: "72px" }} /* clears the fixed header height */
        >
          {/* Scrollable menu body */}
          <div className="flex-1 overflow-y-auto px-6 py-6 space-y-5">
            <HashLink
              to="/#"
              className="block text-lg font-medium py-1 border-b border-white/10"
            ...
            <div className="border-b border-white/10 pb-1">
              <button
                ...
                className="flex justify-between w-full items-center text-lg font-medium py-1"
              >
                Services
                <FiChevronDown
                  size={20}
              ...
              {mobileServicesOpen && (
                <div className="pl-4 space-y-3 mt-3 mb-2 border-l-2 border-[#e80202]">
                  {services.map((item, index) => (
                    <Link
                      key={index}
                      to={item.path}
                      className="block text-gray-400 hover:text-white text-base transition"
                      onClick={handleMobileMenuClick}
                    >
                      {item.name}
                    </Link>
            ...
            <div className="pt-4 pb-8">
              <HashLink to="/#contact" onClick={handleMobileMenuClick}>
                <button className="w-full px-7 py-3.5 bg-linear-to-br from-[#e80202] to-[#a80000] text-white font-bold rounded-lg text-base shadow-[0_6px_25px_rgba(255,0,0,0.4)] hover:opacity-90 transition-all duration-300">
                  Get a Quote
                </button>
              </HashLink>
            </div>
